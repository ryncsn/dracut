...
int init_module(const char *module_image, const unsigned long len, const char *param_values)
{
	return INLINE_SYSCALL (init_module, 3, module_image, len, param_values);
}

int finit_module(const int fd, const char *param_values, const int flags)
{
	return INLINE_SYSCALL (finit_module, 3, fd, param_values, flags);
}

int open (const char *file, const int oflag, const int mode)
{
...
int mount(const char *source, const char *target, const char *fs, unsigned long flags,
          const void *data)
{
	return INLINE_SYSCALL (mount, 5, source, target, fs, flags, data);
}

int ioctl(int fd, unsigned long request, const void *arg)
{
	return INLINE_SYSCALL (ioctl, 3, fd, request, arg);
}

void execve(const char *filename, const char *args[], const char *env[])
{
	INLINE_SYSCALL (execve, 3, filename, args, env);
}

int unlink(const char *filename)
{
	return INLINE_SYSCALL (unlink, 1, filename);
}
